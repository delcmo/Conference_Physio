...
\item Original model developed in the late 1970s-early 1980s with continued enhancements since then.
\end{itemize}
...
\hspace{0.5cm} $\bullet$ Have to use \tcr{implicit} time stepping \\
...
\end{equation}
If $\mu_{\max}$ and $\kappa_{\max}$ are used, the discretization is equivalent to a first-order Godunov scheme.
\end{block}
%
...
\item To remain valid and accurate in for supersonic flows
\end{enumerate}
...
\end{equation}
\begin{equation}
\partial_t \left( \rho u \right) + \nabla \cdot \left( \rho u^2 + P \right) =  \tcb{- \frac{A}{2 D_h} \rho f |u| u - \rho g }
\end{equation}
\begin{equation}
\partial_t \left( \rho E \right) + \nabla \cdot \left[ u  \left( \rho E + P \right) \right] =\tcb{ a_w h_w \left( T - T_w \right) - \rho g u}
\end{equation}
\end{subequations}
\end{block}
...
\item Wall friction, gravity, and wall heat source $Q_w = h_t p_w (T_{liq} - T_w)$ % with $h_t = 5.33 \times 10^4$ $W/m$, $p_w = 2.82$ $mm$.
